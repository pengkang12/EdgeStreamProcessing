...
case "$option" in
   ...
   "amnis") echo "using amnis scheduler"
 	# Amnis method
	topologyMap="spout1:master,SenMLParseBoltPRED:edge2,DecisionTreeClassifyBolt:edge2,LinearRegressionPredictorBolt:edge2,BlockWindowAverageBolt:edge2,MQTTPublishBolt:worker1,ErrorEstimationBolt:worker1,sink:core"
   ;;
   ...
   "beaver") echo "using beaver scheduler"
	...
	topologyMap="spout1:master,SenMLParseBoltPRED:edge2,DecisionTreeClassifyBolt:worker1,LinearRegressionPredictorBolt:edge2,BlockWindowAverageBolt:worker1,MQTTPublishBolt:core,ErrorEstimationBolt:core,sink:core"
   ;;
 
esac
echo "$inputrate $topologyMap"

topologyMap=`echo $topologyMap | sed -e "s/:/:storm-/g" | sed -e "s/,/-predict-taxi,/g" | sed -e "s/$/-predict-taxi/g"`
echo "$topologyMap"
